 ...
 */
@Data
@Schema(description = "获取防作弊证据短期上传凭证请求")
public class AntiCheatEvidenceUploadTicketReq {

    @NotBlank(message = "客户端事件 ID 不能为空")
    @Size(max = 64, message = "客户端事件 ID 不能超过 64 个字符")
    @Schema(description = "客户端事件幂等 ID，用于后续批量上报时绑定证据",
            example = "evt-3f2a9d8c7b6e")
    private String eventId;

    @NotBlank(message = "文件名不能为空")
    @Size(max = 256, message = "文件名不能超过 256 个字符")
    @Schema(description = "原始文件名（仅用于生成对象键，服务端不信任其扩展名）",
            example = "page-blur-001.jpg")
    ...
    @Schema(description = "文件大小（字节）", example = "524288")
